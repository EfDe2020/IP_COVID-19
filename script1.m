...
for i = 0:Ka-1
    A(i+1) = (D0(J-i)-D0(J-i-JDI))/(I0(J-i-JDI)-I0(J-i-JDI-JAI+1))/Mort_Rate;
end
alpha = mean(A)
...
for i = 1:M-1
    S(i+1) = S(i)-b_*(pC*I(i)+r(delay_ind(i-delay))*E(i))*S(i)/N;
    E(i+1) = (1-sigma)*E(i)+b_*(pC*I(i)+r(delay_ind(i-delay))*E(i))*S(i)/N;
...
if (scenario_p == 1)
    ...
    for i = 2:J-J_id
        E0(i-1) = (I0(i)-(1-gamma_i)*I0(i-1))/sigma;
        if E0(i-1) < 0
            if i > 2
                E0(i-1) = E0(i-2);
            else
    ...
    subplot(3,1,1); plot(t(K+1:J-J_id),G,'m',t(K+1:J-J_id),gamma_i*ones(J-J_id-K,1),'m:'); title('\gamma'); xlim([t(K+1) t(J-J_id)]);
    subplot(3,1,2); plot(t(K+1:J-J_id),B,'r',t(K+1:J-J_id),b_i*ones(J-J_id-K,1),'r:'); title('b'); xlim([t(K+1) t(J-J_id)]);
    subplot(3,1,3); plot(t(2:J-J_id),dI0,'c',t(2:J-J_id),[mean(dI0(1:i_min-1))*ones(1,i_min-1) mean(dI0(i_min:J-J_id-1))*ones(1,J-J_id-i_min)],'c--'); title('\tau'); xlim([t(2) t(J-J_id)]);
else
    ...
    b_i = mean(B(B>0))
    figure('Name',['Identified parameters for ' Country],'Position',Pos);
    subplot(3,1,1); plot(t(J-J_id-Ki-1:J-J_id-1),G,'m',t(J-J_id-Ki-1:J-J_id-1),gamma_i*ones(Ki+1,1),'m:'); title('\gamma'); xlim([t(J-J_id-Ki-1) t(J-J_id-1)]);
    subplot(3,1,2); plot(t(J-J_id-Ki-1:J-J_id-1),B,'r',t(J-J_id-Ki-1:J-J_id-1),b_i*ones(Ki+1,1),'r:'); title('b'); xlim([t(J-J_id-Ki-1) t(J-J_id-1)]);
    subplot(3,1,3); plot(t(2:J-J_id),dI0,'c',t(2:J-J_id),[mean(dI0(1:i_min-1))*ones(1,i_min-1) mean(dI0(i_min:J-J_id-1))*ones(1,J-J_id-i_min)],'c--'); title('\tau'); xlim([t(2) t(J-J_id)]);
end
...
if (x < M-J-J_id+2)
    ...
    text(x, s, ['I_{peak} = ' num2str(floor(y))]);
end
...
if (x < M-J-J_id+2)
    x = t(x+J-J_id-2);
    line('XData', [x x t(1)],'YData',[1 y y],'LineWidth',1,'LineStyle', ':','Color','r');
    text(x, s, ['I_{peak}^{optimiste} = ' num2str(floor(y))]);
end
