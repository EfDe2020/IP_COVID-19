function script2(deltap,deltam,delta_IC)
...
I0 = alpha1*(I0-D0-H0); R0 = alpha2*H0; % measured recovered 
...
for k = 0:J-J_id-1-K
    ...
    for i = 1:K
        den = den+I0(k+i)^2;
        num = num+(D0(k+i+1)-D0(k+i))*I0(k+i);
    end
    W(k+1) = num/den;
end
mu_i = mean(W(W>0))
...
for i = 2:J-J_id
    E0(i-1) = (I0(i)-(1-mu_i)*I0(i-1)+R0(i)-R0(i-1))/sigma/(1+kappa);
    if (i > 2)&&(E0(i-1) < 0)
        E0(i-1) = E0(i-2);
    elseif (i == 2)&&(E0(1) < 0)
        E0(1) = 1;
    end
    S0(i-1) = N-R0(i-1)-I0(i-1)-E0(i-1)-D0(i-1);
end
...
if (scenario_p == 1)
    ...
    for k = 0:J-J_id-1-K
        ...
        for i = 1:K
            den = den+I0(k+i)^2;
            num = num+(R0(k+i+1)-R0(k+i)-kappa*I0(k+i+1)+kappa*(1-mu_i)*I0(k+i))*I0(k+i);
        end
        G(k+1) = num/den/(1+kappa);
    end
    gamma_i = mean(G(G>0))
    for k = 0:J-J_id-1-K
        den = 0; num = 0;
        for i = 2:K
            ...
            num = num+(E0(k+i)-(1-(1+kappa)*sigma)*E0(k+i-1))*S0(k+i-1)*(p(k+i-1)*I0(k+i-1)+r(k+i-1)*E0(k+i-1));
        end
        B(k+1) = N*num/den;
    end
    b_i = mean(B(B>0))
    ...
    for k = Ki:-1:0
        ...
        for i = 1:J-J_id-k-1
            den = den+I0(i)^2;
            num = num+(R0(i+1)-R0(i)-kappa*I0(i+1)+kappa*(1-mu_i)*I0(i))*I0(i);
        end
        G(Ki-k+1) = num/den/(1+kappa);
    end
    gamma_i = mean(G(G>0))
    for k = Ki:-1:0
        den = 0; num = 0;
        for i = 2:J-J_id-k-1
            ...
            num = num+(E0(i)-(1-(1+kappa)*sigma)*E0(i-1))*S0(i-1)*(p(i-1)*I0(i-1)+r(i-1)*E0(i-1));
        end
        B(Ki-k+1) = N*num/den;
    end
    b_i = mean(B(B>0))
...
subplot(2,2,3); plot(t(K+1:J-J_id),W,'c',t(K+1:J-J_id),mu_i*ones(J-J_id-K,1),'c:'); title('\mu'); xlim([t(K+1) t(J-J_id)]);
%plot(t(2:J-J_id),dI0,'c',t(2:J-J_id),[mean(dI0(1:i_min-1))*ones(1,i_min-1) mean(dI0(i_min:J-J_id-1))*ones(1,J-J_id-i_min)],'c--'); title('\tau 1 method'); xlim([t(2) t(J-J_id)]);
...
R = ones(M,1); E = ones(M,1); I = ones(M,1); S = ones(M,1); D = ones(M,1);
R(1) = R0(1); I(1) = I0(1); E(1) = E0(1); S(1) = S0(1); D(1) = D0(1);

for i = 1:M-1
    S(i+1) = S(i)-b_i*(p(i)*I(i)+r(i)*E(i))*S(i)/N;
    E(i+1) = (1-(1+kappa)*sigma)*E(i)+b_i*(p(i)*I(i)+r(i)*E(i))*S(i)/N;
    I(i+1) = (1-gamma_i-mu_i)*I(i)+sigma*E(i);
    R(i+1) = R(i)+gamma_i*I(i)+kappa*sigma*E(i);
    D(i+1) = D(i)+mu_i*I(i);
end

figure('Name',['Simulation with identified data in ' Country],'Position',Pos);
semilogy(t,E,'r',t,I,'b',t,R,'g',t,D,'c',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); legend('E','I','R','D','Location','southeast');
ax = gca; set(ax,'xTick',ticks,'XGrid','on');
...
subplot(2,2,4); semilogy(t(1:J-J_id-1),D(1:J-J_id-1),'m',t(1:J-J_id-1),D0(1:J-J_id-1),'m--'); title('D'); xlim([t(1) t(J-J_id-1)]);
...
gamma_ = gamma_i*delta_; b_ = b_i*delta_; r_ = r.*delta_; p_ = p.*delta_; sigma_ = sigma*delta_; mu_ = mu_i*delta_;
Rm = zeros(M,1); RM = zeros(M,1); Im = zeros(M,1); IM = zeros(M,1); Dm = zeros(M,1); DM = zeros(M,1);
Em = zeros(M,1); EM = zeros(M,1); Sm = zeros(M,1); SM = zeros(M,1);
% inirial conditions
Rm(1) = delta_(1)*R(1); RM(1) = delta_(2)*R(1);
Dm(1) = delta_(1)*D(1); DM(1) = delta_(2)*D(1);
...
for i = 1:M-1
    ...
    Em(i+1) = (1-(1+kappa)*sigma_(2)+b_(1)*r_(i,1)*Sm(i)/N)*Em(i)+b_(1)*p_(i,1)*Im(i)*Sm(i)/N;
    SM(i+1) = min(N,(1-b_(1)*(p_(i,1)*Im(i)+r_(i,1)*Em(i))/N)*SM(i));
    EM(i+1) = min(N,(1-(1+kappa)*sigma_(1)+b_(2)*r_(i,2)*SM(i)/N)*EM(i)+b_(2)*p_(i,2)*IM(i)*SM(i)/N);
    Im(i+1) = (1-gamma_(2))*Im(i)+sigma_(1)*Em(i);
    Rm(i+1) = Rm(i)+gamma_(1)*Im(i)+kappa*sigma_(1)*Em(i);
    Dm(i+1) = Dm(i)+mu_(1)*Im(i);
    IM(i+1) = min(N,(1-gamma_(1))*IM(i)+sigma_(2)*EM(i));
    RM(i+1) = min(N,RM(i)+gamma_(2)*IM(i)+kappa*sigma_(2)*EM(i));
    DM(i+1) = min(N,DM(i)+mu_(2)*IM(i));
end

figure('Name',['Simulation of interval predictor for deviation of all parameters for ' Country],'Position',Posl);
subplot(2,2,1); semilogy(t,E,'r',t,Em,'r:',t,EM,'r--',t(1:J-J_id-1),E0,'ro',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) max(max(EM),max(E0))]); title('E');
ax = gca; set(ax,'xTick',ticks,'XGrid','on');
subplot(2,2,2); semilogy(t,I,'b',t,Im,'b:',t,IM,'b--',t(1:J-J_id),I0(1:J-J_id),'bo',t(J-J_id+1:J),I0(J-J_id+1:J),'bs',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) max(max(IM),max(I0))]); title('I');
ax = gca; set(ax,'xTick',ticks,'XGrid','on');
subplot(2,2,3); semilogy(t,R,'g',t,Rm,'g:',t,RM,'g--',t(1:J-J_id),R0(1:J-J_id),'go',t(J-J_id+1:J),R0(J-J_id+1:J),'gs',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) max(max(RM),max(R0))]); title('R');
ax = gca; set(ax,'xTick',ticks,'XGrid','on');
subplot(2,2,4); semilogy(t,D,'m',t,Dm,'m:',t,DM,'m--',t(1:J-J_id),D0(1:J-J_id),'mo',t(J-J_id+1:J),D0(J-J_id+1:J),'ms',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) max(max(DM),max(D0))]); title('D');
%semilogy(t,S,'m',t,Sm,'m:',t,SM,'m--',t(1:J-J_id-1),S0,'mo',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); title('S');
ax = gca; set(ax,'xTick',ticks,'XGrid','on');
...
if (Type == 1)
    ...
    plot(t(1:J-J_id),I0(1:J-J_id),'bo',t(J-J_id+1:J),I0(J-J_id+1:J),'bs'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) inf]); title('I'); %,t,N*ones(M,1),'k:'
end
...
Dm(J-J_id-1) = delta_(1)*D0(J-J_id-1); DM(J-J_id-1) = delta_(2)*D0(J-J_id-1);
...
R_ = zeros(M,1); E_ = zeros(M,1); I_ = zeros(M,1); S_ = zeros(M,1); D_ = zeros(M,1);
E_(J-J_id-1) = E0(J-J_id-1); R_(J-J_id-1) = R0(J-J_id-1); I_(J-J_id-1) = I0(J-J_id-1); S_(J-J_id-1) = S0(J-J_id-1); D_(J-J_id-1) = D0(J-J_id-1);

for i = J-J_id-1:M-1
    S_(i+1) = S_(i)-b_i*(p(i)*I_(i)+r(i)*E_(i))*S_(i)/N;
    E_(i+1) = (1-(1+kappa)*sigma)*E_(i)+b_i*(p(i)*I_(i)+r(i)*E_(i))*S_(i)/N;
    I_(i+1) = (1-gamma_i)*I_(i)+sigma*E_(i);
    R_(i+1) = R_(i)+gamma_i*I_(i)+kappa*sigma*E(i);
    D_(i+1) = D_(i)+mu_i*I_(i);
    Sm(i+1) = (1-b_(2)*(p_(i,2)*IM(i)+r_(i,2)*EM(i))/N)*Sm(i);
    Em(i+1) = (1-(1+kappa)*sigma_(2)+b_(1)*r_(i,1)*Sm(i)/N)*Em(i)+b_(1)*p_(i,1)*Im(i)*Sm(i)/N;
    SM(i+1) = min(N,(1-b_(1)*(p_(i,1)*Im(i)+r_(i,1)*Em(i))/N)*SM(i));
    EM(i+1) = min(N,(1-(1+kappa)*sigma_(1)+b_(2)*r_(i,2)*SM(i)/N)*EM(i)+b_(2)*p_(i,2)*IM(i)*SM(i)/N);
...
% gamma_ = gamma_i*[1 1]; b_ = b_i*[1 1]; r_ = r.*[1 1]; p_ = p.*[1 1]; sigma_ = [1/12 1/2]; mu_ = mu_i*[1 1];
% Rm = zeros(M,1); RM = zeros(M,1); Im = zeros(M,1); IM = zeros(M,1); Dm = zeros(M,1); DM = zeros(M,1);
% Em = zeros(M,1); EM = zeros(M,1); Sm = zeros(M,1); SM = zeros(M,1);
% % inirial conditions
% Rm(1) = delta_(1)*R(1); RM(1) = delta_(2)*R(1);
...
% subplot(2,2,1); semilogy(t,E,'r',t,Em,'r:',t,EM,'r--',t(1:J-J_id-1),E0,'ro',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) inf]); title('E');
% ax = gca; set(ax,'xTick',ticks,'XGrid','on');
% subplot(2,2,2); semilogy(t,I,'b',t,Im,'b:',t,IM,'b--',t(1:J-J_id),I0(1:J-J_id),'bo',t(J-J_id+1:J),I0(J-J_id+1:J),'bs',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) inf]); title('I');
% ax = gca; set(ax,'xTick',ticks,'XGrid','on');
% subplot(2,2,3); semilogy(t,R,'g',t,Rm,'g:',t,RM,'g--',t(1:J-J_id),R0(1:J-J_id),'go',t(J-J_id+1:J),R0(J-J_id+1:J),'gs',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) inf]); title('R');
% ax = gca; set(ax,'xTick',ticks,'XGrid','on');
% subplot(2,2,4); semilogy(t,D,'m',t,Dm,'m:',t,DM,'m--',t(1:J-J_id),D0(1:J-J_id),'mo',t(J-J_id+1:J),D0(J-J_id+1:J),'ms',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); i = ylim; ylim([max(1,i(1)) inf]); title('D');
% %semilogy(t,S,'m',t,Sm,'m:',t,SM,'m--',t(1:J-J_id-1),S0,'mo',t,N*ones(M,1),'k:'); xlim([t(1) t(M)]); title('S');
% ax = gca; set(ax,'xTick',ticks,'XGrid','on');